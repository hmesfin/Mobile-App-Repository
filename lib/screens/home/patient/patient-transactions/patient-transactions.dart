...
import 'patient-transactions-provider.dart';

class PatientTransactions extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _PatientTransactionsState();
}
...
class _PatientTransactionsState extends State<PatientTransactions> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<PatientTransactionsProvider>(context, listen: false)
          .getTransactionList(
              Provider.of<PatientTransactionsProvider>(context, listen: false)
                  .page);
      controller.addListener(() {
        if (controller.position.pixels >
          ...
          Provider.of<PatientTransactionsProvider>(context, listen: false)
              .page++;
          Provider.of<PatientTransactionsProvider>(context, listen: false)
              .getTransactionList(Provider.of<PatientTransactionsProvider>(
                      context,
                      listen: false)
                  .page);
        }
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Consumer<PatientTransactionsProvider>(
                builder: (context, transaction, _) => (transaction.isLoading &&
                                        ...
                                        Row(
                                          children: [
                                            ...
                                            Container(
                                              // width: screenWidth / 1.9,
                                              child: Text(
                                                "${DateFormat("dd MMMM yyyy").format(DateTime.parse(transaction.list[index].updatedAt))}",
                                            ...
                                            Container(
                                              // width: screenWidth / 1.9,
                                              child: Text(
                                                "${DateFormat("hh:mm aaa").format(DateTime.parse(transaction.list[index].updatedAt).toLocal())}",
                                                overflow: TextOverflow.ellipsis,
                                                maxLines: 5,
                                                style: TextStyle(
                                                    fontFamily:
